...
# soft_MRR / soft_NDCG@5 are rank-variant and no longer computed: level-2/3 child
# hits are collapsed onto their level-1 parents through a set, which discards the
# fused rank order, so any metric that reads the POSITION of a retrieved chunk is
# measuring an arbitrary permutation. soft_Recall@5 is a set-membership test at the
# retrieval depth and is unaffected.
RETRIEVAL_METRICS = ["soft_Recall@5"]
