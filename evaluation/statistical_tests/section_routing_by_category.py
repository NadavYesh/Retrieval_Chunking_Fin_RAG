...
Only rank-invariant retrieval metrics are reported. soft_MRR and soft_NDCG@5 are
both excluded because they read the ORDER of the retrieved chunks, and that order
is not meaningful for the parent-fetch configs: level-2/3 child hits are collapsed
onto their level-1 parents through a set, and the parents are then re-fetched by id
(Qdrant returns points by id in no guaranteed order), so the rank a chunk lands at
is arbitrary. soft_Recall@5 is a pure set-membership test at the retrieval depth --
did any retrieved chunk clear the relevance threshold -- so it is unaffected.
...
list and the ratio could never see whether any chunk was actually relevant.
