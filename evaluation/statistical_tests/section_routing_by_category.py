...
Multiplicity
  ...
  Holm's step-down procedure controls the same family-wise error rate as
  Bonferroni and is uniformly more powerful, so Holm is used. Benjamini-Hochberg
  is reported alongside for readers who prefer FDR control.
...
def category_report(trials: pd.DataFrame) -> pd.DataFrame:
    ...
    # Family-wise correction across the eight categories (see module docstring).
    for raw_col, method, adj_col, sig_col in [
